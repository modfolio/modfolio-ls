...
export const GET: APIRoute = async ({ url, cookies, redirect }) => {
	...
	const verifier = cookies.get("ml_pkce_verifier")?.value;
	const returnTo = cookies.get("ml_return_to")?.value ?? "/";

	cookies.delete("ml_pkce_verifier", { path: "/" });
	cookies.delete("ml_oauth_state", { path: "/" });
	cookies.delete("ml_return_to", { path: "/" });
